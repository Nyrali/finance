...
def generate_resizable_stacked_chart_html(df, categories, output_file="D:/finance/resizable_stacked_chart.html"):
    import pandas as pd
    ...
    for i, cat in enumerate(categories):
        values = [
            grouped[(grouped['year_month'] == m) & (grouped['category'] == cat.lower())]['debit'].sum()
            for m in labels
        ]
        datasets.append(f"""{{
...
<body>
  <h2>Monthly Debit by Category (Resizable + Stacked)</h2>
    ...
    const debitChart = new Chart(ctx, {{
        ...
        options: {{
            ...
            plugins: {{
                ...
                title: {{
                    display: true,
                    text: 'Monthly Debit by Category (Stacked)',
                    font: {{ size: 22 }}
                }}
            }},
            scales: {{
...
"""
    ...
    print(f"✅ Resizable Chart HTML saved as {output_file}")
...
def get_color_palettes(opacity=0.85):
    ...
    return {
        "default": [
            rgba(54, 162, 235), rgba(255, 99, 132), rgba(255, 206, 86),
            rgba(75, 192, 192), rgba(153, 102, 255), rgba(255, 159, 64),
            rgba(201, 203, 207), rgba(0, 204, 102), rgba(255, 102, 255),
            rgba(102, 255, 255), rgba(0, 0, 0), rgba(240, 100, 100),
            rgba(100, 240, 100), rgba(100, 100, 240)
        ],
        "forest": [
            rgba(253, 216, 167), rgba(150, 128, 108), rgba(136, 107, 85),
            rgba(46, 75, 71), rgba(35, 50, 45)
        ],
        "pastel": [
            rgba(255, 179, 186), rgba(255, 223, 186), rgba(255, 255, 186),
            rgba(186, 255, 201), rgba(186, 225, 255)
        ],
        "autumn_night": [
...
def normalize_categories(df, categories):
    ...
    df['category_high_lvl'] = df['category_high_lvl'].astype(str).str.strip().str.lower()
...
def prepare_pivoted_data(df, categories):
    ...
    grouped = filtered.groupby(['year_month', 'category_high_lvl'])['debit'].sum().reset_index()
    return grouped.pivot(index='year_month', columns='category_high_lvl', values='debit').fillna(0)

def sort_categories(pivoted):
    base_order = [
        "bydlení", "restaurace / potraviny", "doprava",
        "zábava a volný čas", "Oblečení a styl", "jiné finanční výdaje"
    ]
    ...
    for idx, row in pivoted.iterrows():
        ...
        sorted_row = row.reindex(row_order + [col for col in pivoted.columns if col not in row_order], fill_value=0)
...
def export_transaction_data(df, categories):
    display_cols = ['year_month', 'booking_date', 'debit', 'category_high_lvl', 'counter_acc_name', 'counter_acc', 'category']
    filtered = df[df['category_high_lvl'].isin(categories)]
...
def build_html(labels, categories, data_matrix, transactions_json, palettes, opacity=0.95):
    import json
...
def patron_generate_resizable_stacked_chart_html_1(df, categories, output_file="D:/finance/patron_chart_1.html", opacity=0.90):
    try:        
