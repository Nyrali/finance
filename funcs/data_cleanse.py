...
def remove_unwanted_transactions(df, name_list):
    return df[
        ~df["counter_acc_name"].str.lower().isin([name.lower() for name in name_list])
    ]
...
def data_cleanse(df):
    ...
    df["booking_date"] = pd.to_datetime(
        df["booking_date"], format="%d.%m.%Y", errors="coerce"
    )
    ...
    exclude_names = ["Tkáč Rudolf", "TKAC RUDOLF", "Ruda Csob"]
    ...
    df["category"] = (
        df["counter_acc_name"]
        .map(category_mapping_counter_acc_name)
        .fillna(df["category"])
    )
    ...
    df["category"] = (
        df["counter_acc"].map(category_mapping_counter_acc).fillna(df["category"])
    )
